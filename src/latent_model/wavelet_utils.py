...
class WaveletData(object):
    def __init__(self, shape_list, output_stage, max_depth, data_stage=None, low=None, highs_values=None,
                 highs_indices=None, wavelet_volume=None):
        ...
        self.highs_indices = highs_indices[..., 1:] if highs_indices is not None else highs_indices # extract only the last three dimension
    ...
    def convert_wavelet_volume(self):
        ...
        if self.wavelet_volume is not None:
            ...
            if self.output_stage == 0:
                ...
                ## assertion
                assert self.low.size(0) == self.highs_values.size(0) and self.highs_values.size(
                    0) == self.highs_indices.size(0)
                ...
                highs_volume = torch.zeros((batch_size, self.shape_list[-1][0],
                                            self.shape_list[-1][1],
                                            self.shape_list[-1][2], data_dim)).to(
                    self.low.device)  # output high volumes

                ### extend batch_size to the indices
                batch_pad = torch.arange(batch_size, device=self.highs_indices.device).unsqueeze(1).repeat(
                    (1, self.highs_indices.size(1))).unsqueeze(2).long()  # B * P * 1
                high_indices_filled = torch.cat((batch_pad, self.highs_indices), dim=2)  # B * P * 4

                ### fill the values
                high_indices_filled = high_indices_filled.reshape((-1, 4))
                highs_volume[high_indices_filled[:, 0], high_indices_filled[:, 1], high_indices_filled[:, 2],
                high_indices_filled[:, 3], :] = self.highs_values.reshape((-1, data_dim))
    ...
    def convert_low_highs(self):
        ...
        if self.output_stage == 0:  # fill with empty zeros
            ...
            highs = batch_extract_highs_from_values(output_stage=self.output_stage, max_depth=self.max_depth,
                                                    shape_list=self.shape_list, device=low.device, batch_size=batch_size)
        else:
            highs_volume = wavelet_volume[:, 1:]

            output_dim = 8 ** self.output_stage - 1

            highs_indices = extract_full_indices(device=wavelet_volume.device, max_depth=self.max_depth,
                                                 shape_list=self.shape_list)  # N * 511 * 4 (N = 46^3)
            highs_indices = highs_indices[:, -output_dim:]  # N * O * 4 (N = 46^3)

            # padding
            batch_size = wavelet_volume.size(0)
            highs_indices = padding_high_indices(batch_size, highs_indices)  # B * N * O * 5
            ...
            ## obtain the highs
            highs = batch_extract_highs_from_values(output_stage=self.output_stage, max_depth=self.max_depth,
                                                    shape_list=self.shape_list, device=wavelet_volume.device,
                                                    highs_indices=highs_indices, highs_values=highs_values,
                                                    batch_size=batch_size)

        return low, highs


def batch_extract_highs_from_values(output_stage, max_depth, shape_list, device, batch_size, highs_indices=None, highs_values=None):
    cnt = 0
    highs_recon = []
    for idx in range(max_depth):
        current_stage = (max_depth - 1) - idx

        padding_size = (shape_list[-1][0] * (2 ** current_stage) - shape_list[idx + 1][0]) // 2
        high_new = torch.zeros((batch_size, 1, 7, shape_list[idx + 1][0] + padding_size * 2,
                                shape_list[idx + 1][1] + padding_size * 2,
                                shape_list[idx + 1][2] + padding_size * 2)).type(highs_values.type()).to(device)
        ...
        if current_stage < output_stage:
            ...
            # set the values
            high_new[high_indices_idx[:, 0], 0, high_indices_idx[:, 1], high_indices_idx[:, 2], high_indices_idx[:,
                                                                                                3], high_indices_idx[:,
                                                                                                    4]] = high_values_idx
            cnt += indices_len
...
def padding_high_indices(batch_size, highs_indices):
    ...
    batch_size_pad = batch_size_pad.repeat(1, highs_indices.size(0), highs_indices.size(1),
                                           1)  # B * N * O * 1 (N = 46^3)
    highs_indices = highs_indices.unsqueeze(0).repeat(batch_size, 1, 1, 1)  # B * N * O * 4 (N = 46^3)
...
def extract_highs_from_colors(high_indices, high_values, max_depth, shape_list):
    ...
    for idx in range(max_depth):
        ...
        high_new = torch.zeros((1, 7, shape_list[idx + 1][0] + padding_size * 2,
                                shape_list[idx + 1][1] + padding_size * 2, shape_list[idx + 1][2] + padding_size * 2,
                                3)).to(high_indices_idx.device)
        ...
        # set the values
        high_new[0, high_indices_idx[:, 0], high_indices_idx[:, 1], high_indices_idx[:, 2], high_indices_idx[:, 3],
        :] = high_values_idx
...
def extract_highs_from_values(high_indices, high_values, max_depth, shape_list):
    ...
    for idx in range(max_depth):
        ...
        high_new = torch.zeros((1, 1, 7, shape_list[idx + 1][0] + padding_size * 2,
                                shape_list[idx + 1][1] + padding_size * 2,
                                shape_list[idx + 1][2] + padding_size * 2)).to(high_indices_idx.device)
        ...
        # set the values
        high_new[0, 0, high_indices_idx[:, 0], high_indices_idx[:, 1], high_indices_idx[:, 2], high_indices_idx[:,
                                                                                               3]] = high_values_idx
...
def create_coordinates(resolution, space_range, channel_dim=7):
    ...
    ch, x, y, z = ch[:, :, :, :, np.newaxis], x[:, :, :, :, np.newaxis], y[:, :, :, :, np.newaxis], z[:, :, :, :,
                                                                                                    np.newaxis]
