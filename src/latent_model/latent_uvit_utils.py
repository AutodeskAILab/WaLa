...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, out_channels=None, use_conv=False, dims=2, output_res=None):
        super().__init__()
        self.channels = channels
        self.use_conv = use_conv
        self.dims = dims
...
class Latent_UVIT(nn.Module):
    ...
    def __init__(
        ...
        if add_condition_res_ch is not None:
            self.res_cond_proj_layer = Global_Proj_Layer(context_dim, add_condition_res_ch, hidden_dim=hidden_size, activation=nn.SiLU())
        ...
        self.to_patch_att_embedding_input = nn.Sequential(
                                                ...
                                                nn.Linear(patch_size*patch_size*patch_size*unet_model_channels, hidden_size)
                                                )
        ...
        if with_fix_pos is not None:
            self.pos_emb = nn.Parameter(torch.zeros(1, total_att_seq, hidden_size), requires_grad=False)
        else:    
            self.pos_emb = nn.init.trunc_normal_(nn.Parameter(torch.zeros(1,total_att_seq, hidden_size)), 0., 0.01)
        ...
        self.to_patch_att_embedding_output = nn.Sequential(
                                                    ...
                                                    nn.Linear(hidden_size, patch_size*patch_size*patch_size*unet_model_channels),
                                                    Rearrange('b (h w l) (p1 p2 p3 c) -> b c (h p1) (w p2) (l p3)', p1=patch_size, p2=patch_size, p3=patch_size, h=att_size, w=att_size, l=att_size)
                                                )
        ...
        self.out = nn.Sequential(
            ...
            nn.SiLU(),
            zero_module(conv_nd(3, unet_model_channels, self.out_channels, 3, padding=1)),
        )
    ...
    def forward(self, x, t, latent_codes=None):
        ...
        if self.add_num_register > 0:
            #print(h.shape)
            h = torch.cat([h, self.register_emb.repeat(h.size(0),1,1).type(h.type())], dim=1)
            #print(h.shape)
