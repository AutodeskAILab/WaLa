...
class Trainer_Condition_Network_new(pl.LightningModule):
    ...
    def forward(self, images, low,data_idx):
            # Generate prediction and save visualization
            with torch.no_grad():
    ...
    def extract_input_features(self, images,low,data_idx, data_type, is_train=False, to_cuda=False):
        """Extract input features from the data."""

        image = images.type(torch.FloatTensor).cuda()
        input_features = self.extract_input_image_features(image, is_train=is_train)
